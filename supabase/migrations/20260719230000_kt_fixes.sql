...
--    it with the next free number instead.
--    The duplicate guard must live in HAVING, not WHERE: a WHERE clause that
--    is false empties the aggregate's INPUT (max -> null -> id '1' -> unique
--    violation on a rebuilt database where 20260719210000 already inserted
--    the row); HAVING filters the aggregate's OUTPUT, making this a true
--    no-op when the line already exists. (Found rebuilding Kc-production.)
