...
//
// The Revolut Business export (the shop's likely bank) gets explicit aliases:
// "Date completed (UTC)" / "(Europe/London)" for the date (completed beats
// started — a started-but-declined payment never moved money), "Payer" for the
// counterparty, and its own "ID", "Fee" and "State" columns handled below.
const COLUMN_ALIASES = [
  ['date',         ['date completed europe london', 'date completed utc', 'date completed',
                    'transaction date', 'value date', 'completed date', 'posting date', 'date value', 'date']],
  ...
  ['counterparty', ['counter party', 'counterparty', 'payer', 'payee', 'to from', 'name']],
  ['description',  ['transaction description', 'description', 'narrative', 'details', 'reference', 'memo', 'transaction', 'type']],
  // A second free-text column alongside Description (Revolut has both). Listed
  // AFTER description so a file with only "Reference" still gets a description
  // via the alias above; when both exist the two are merged at parse time.
  ['reference',    ['payment reference', 'reference']],
  // Revolut books the fee separately: the balance change is Amount − Fee.
  ['fee',          ['total fees', 'fee', 'fees']],
  // Lifecycle column — DECLINED/REVERTED rows never moved money.
  ['state',        ['state', 'status']],
  // The bank's own id for the movement — better than synthesising one.
...
export function parseStatementCsv(text, opts = {}) {
  ...
  // Rows whose State says the money never moved. DECLINED and REVERTED rows in
  // a Revolut export still carry an amount; importing them would invent
  // movements the balance never saw. PENDING is excluded too — it will arrive
  // again as COMPLETED in the next export, with the same bank id.
  const DEAD_STATES = new Set(['declined', 'reverted', 'failed', 'cancelled', 'canceled', 'pending', 'rejected'])

  const parsed = []
  let skipped = 0
  let notCompleted = 0
  for (const r of body) {
    const state = String(cell(r, columns.state) || '').trim().toLowerCase()
    if (state && DEAD_STATES.has(state)) { notCompleted++; continue }
    ...
    // Revolut books its fee beside the amount, not inside it: the balance
    // change is Amount − Fee. Folding it in here keeps every stored amount
    // meaning the same thing — what actually happened to the account.
    if (columns.fee != null) {
      const fee = parseAmount(cell(r, columns.fee))
      if (fee != null && fee !== 0) amount = Math.round((amount - Math.abs(fee)) * 100) / 100
      if (amount === 0) { skipped++; continue } // a fee that consumed the whole movement
    }

    // Description and Reference are both free text and either can hold the part
    // that identifies the payer (a Wizz PNR usually rides in Reference), so
    // when both exist they are kept together.
    const descMain = String(cell(r, columns.description) || '').trim()
    const descRef = String(cell(r, columns.reference) || '').trim()
    const description = [descMain, descRef].filter(Boolean).join(' · ')
    const counterpartyName = String(cell(r, columns.counterparty) || '').trim() || null
    const bankId = String(cell(r, columns.txnId) || '').trim()
  ...
  if (notCompleted) warnings.push(`${notCompleted} row${notCompleted === 1 ? '' : 's'} skipped — declined, reverted or still pending, so no money moved.`)

  // The bank's own id wins when the file has one (scoped per account label so
  // two uploads of different accounts can never collide); otherwise assign
  // occurrence indices across identical movements, in file order.
  const seen = new Map()
  const rows = parsed.map(({ bankId, ...row }) => {
    if (bankId) return { ...row, providerTxnId: `bank:${digest(accountRef)}:${bankId}` }
