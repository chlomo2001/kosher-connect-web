...
// One mailbox, however it is spelled. Same rule as the app's normalizeEmail and
// lib/mappers' — lowercase, drop full stops from the name part, cut anything
// after a "+" — so a payer who checked out as e.a.rothbart+shop@ still matches
// earothbart@ on file. Returns '' for anything that isn't an address, which can
// never equal another '' because the caller checks both sides are truthy first.
...
export function scoreCandidate(txn, candidate, opts = {}) {
  ...
  // 2b. Email. A bank line never carries one; a card checkout almost always
  // does, and it is the strongest identifier in the whole set — the payer typed
  // it themselves. Matched on the app's own mailbox rule (dots and +tags are
  // decoration), so earothbart@ and e.a.rothbart@ are one address. Scored just
  // under a reference: an address can be a spouse's or a gabbai's, so it
  // proposes a person, it does not prove one.
  if (txn.email && candidate.email && mailboxKey(txn.email) === mailboxKey(candidate.email)) {
    score += 50
    reasons.push(`paid from ${candidate.email}, the address on file`)
  }

  // 3. Name. Reuses the transliteration-aware matcher, so Teitelbaum on the
  // statement still finds Taitelbaum on file.
  //
  // namesSimilar returns an OBJECT — { match, score, strong } — and this line
  // used to test it directly. An object is always truthy, so every candidate
  // scored the name points and every proposal carried "counterparty resembles
  // <whoever>", including SAINSBURYS against a customer. The name, which is
  // the signal a human reads first, was the one signal that distinguished
  // nobody: obvious matches went unfound and unrelated ones were proposed
  // with a name reason attached. (Owner, 24 Aug, on the Stripe pull.)
  //
  // Graded now, because "matched" is not one thing. Two strong tokens is a
  // forename AND a surname — enough to reach 'possible' on its own, which is
  // what a person means by "it is obviously him". One strong token is a
  // surname, which in this community is shared by many families, so it is
  // worth about what an exact amount is worth and no more.
  const nameHit = txn.counterpartyName && candidate.name
    ? namesSimilar(txn.counterpartyName, candidate.name)
    : null
  if (nameHit && nameHit.match) {
    const full = nameHit.strong >= 2
    score += full ? 45 : 25
    reasons.push(full
      ? `counterparty is ${candidate.name} — forename and surname both match`
      : `counterparty shares a name with ${candidate.name}`)
  }
