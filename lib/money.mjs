...
// ---- payment tender (mirrors the payment_method enum) ----
// The ledger's `method` column is a Postgres enum, so an unrecognised string
// does not degrade — it fails the whole insert. Anything writing a method
// checks it against this set first and falls back to null, because a payment
// recorded with an unknown tender is a much smaller problem than a save that
// throws. Keep in step with the enum: cash, card, voucher, bank_transfer,
// wallet, other.
export const PAYMENT_METHODS = new Set([
  'cash', 'card', 'voucher', 'bank_transfer', 'wallet', 'other',
])
...
// 'refund_payout' sits here for its SIGN, not its meaning: it is money leaving
// for the customer, not a charge earned. Anything reading this set for revenue
// (rather than for the sign check it mirrors) must exclude it — see
// ledger_revenue_since, which keeps it out of `charged`.
const LEDGER_DEBIT = new Set([
  ...
  'phone_sale', 'stock_sale', 'virtual_number', 'extra_charge', 'refund_payout',
])
