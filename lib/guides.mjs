...
export function guideTabs(guides = GUIDES) {
...
/**
 * The guides for one screen, then everything else — the order to show someone
 * who has opened the panel without typing anything. Where they are standing is
 * the best guess at what they are stuck on, and a flat list in file order made
 * a helper on the Repairs screen read past renting a phone out to reach it.
 */
export function guidesByScreen(tab, guides = GUIDES) {
  const here = guides.filter(g => g.tab === tab)
  return { here, rest: guides.filter(g => !here.includes(g)) }
}
 ...
 * twice.
 *
 * `boostTab` nudges the guides belonging to the screen the asker is standing
 * on. Deliberately +1 — the weakest signal there is — so it breaks a tie
 * between two equally plausible answers and never beats a real match: someone
 * typing "take a payment" while on Rentals still gets the payment guide.
 *
 ...
 */
export function matchGuides(query, guides = GUIDES, { limit = 5, boostTab = null } = {}) {
  const q = norm(query)
  if (!q) return []
  ...
  const scored = guides.map(g => {
    ...
    // Only ever a tie-break, and only for a guide that already matched
    // something: the screen you are on is a hint, not an answer.
    if (score > 0 && boostTab && g.tab === boostTab) score += 1
...
export function bestGuide(query, guides = GUIDES) {
