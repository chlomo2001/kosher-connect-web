...
// The stored id is VERIFIED, not trusted: ids minted while the account ran
// test keys (cus_… that "exists in test mode") 404 under live keys, and every
// payment/mandate flow died on them one call later with a bare 500. A dead id
// is replaced with a fresh live customer — callers already persist the id
// they get back, so the heal sticks after the first use.
export async function getOrCreateCustomer({ existingId, email, name, appCustomerId }) {
  if (existingId) {
    try {
      const existing = await stripeApi(`customers/${encodeURIComponent(existingId)}`, { method: 'GET' })
      if (!existing.deleted) return existingId
    } catch { /* no such customer in this mode — fall through and mint a live one */ }
  }
...
/**
 * The same thing, CARD ONLY, for the form the counter fills in itself.
 *
 * Owner, 17 Aug: saving a card opened Stripe's hosted page, which offered Link
 * — and because the customer's address was already enrolled, Stripe emailed
 * them a verification instead of just taking the card. From the counter that
 * is not a feature, it is the shop asking the customer to go and do something.
 *
 * Two differences from the intent above, and both are the point:
 *   · payment_method_types = card. `automatic_payment_methods` is what turns
 *     on Link, wallets and the email that goes with them.
 *   · no hosted page. The client mounts Stripe's own card field in one of our
 *     modals, so the number is still typed into Stripe's iframe and still
 *     never touches this app, this network or these logs — the PCI position is
 *     unchanged — but nobody leaves the screen and nobody is emailed.
 *
 * Recorded by the same setup_intent.succeeded webhook as every other route.
 */
export async function createCardSetupIntent({ customerId, appCustomerId }) {
  return stripeApi('setup_intents', {
    body: {
      customer: customerId,
      usage: 'off_session',
...
// Set up a Bacs Direct Debit mandate (bank account, not a card) for later
// off-session collections — DD phase 1 (docs/DD-PLAN-2026-08-12.md).
// HOSTED CHECKOUT in setup mode, not a bare SetupIntent: Stripe refuses
// direct SetupIntent mandate creation for Bacs on standard accounts
// ("Your account isn't configured to directly use SetupIntents to create
// Mandates for Bacs Direct Debits", live 12 Aug) — the supported route is
// their hosted page, which also renders the Direct Debit Guarantee wording.
// Checkout's underlying SetupIntent fires the same setup_intent.succeeded
// webhook, and setup_intent_data carries our metadata onto it, so the
// mandate-recording path is identical to the card one.
export async function createDdCheckoutSession({ customerId, appCustomerId, successUrl, cancelUrl }) {
  return stripeApi('checkout/sessions', {
    body: {
      ...
      'setup_intent_data[metadata][app_customer_id]': appCustomerId || '',
      'setup_intent_data[metadata][dd_mandate]': '1',
      'metadata[app_customer_id]': appCustomerId || '',
    },
  })
}

// Save a CARD on file from the counter — hosted Checkout in setup mode, the
// same shape as the Bacs one above. Hosted on purpose: the card number is
// typed into Stripe's page on the customer's own phone or the counter tablet
// and never touches this app, this network or these logs, which is the whole
// difference between "we take cards" and "we are in scope for PCI".
// Recorded by the same setup_intent.succeeded webhook — its non-Bacs branch
// writes stripe_pm_id, which is what chargeCardOnFile later spends.
export async function createCardCheckoutSession({ customerId, appCustomerId, successUrl, cancelUrl }) {
  return stripeApi('checkout/sessions', {
    body: {
      mode: 'setup',
      customer: customerId,
...
// Every card payment Stripe holds, oldest-relevant first, for reconciliation.
//
// Read-only, and deliberately the CHARGE list rather than the customer list: a
// customer object says a person exists, a charge says money moved on a date for
// an amount — which is the only thing that can be posted to a wallet. Payment
// Link checkouts mint a fresh guest customer per payment and carry no metadata
// of ours, so their money never reached the app; this is how it is found.
//
// Cursor paging (Stripe's `starting_after`), capped by the caller. `expand` is
// not used: the fields reconciliation needs — amount, created, billing name,
// receipt email, payment_intent, metadata — are all on the charge already.
export async function listCharges({ limit = 100, startingAfter = null, createdGte = null } = {}) {
  const q = new URLSearchParams({ limit: String(Math.min(100, Math.max(1, limit))) })
  if (startingAfter) q.set('starting_after', startingAfter)
  if (createdGte) q.set('created[gte]', String(createdGte))
  const page = await stripeApi(`charges?${q.toString()}`, { method: 'GET' })
  return { charges: page.data || [], hasMore: !!page.has_more }
}
