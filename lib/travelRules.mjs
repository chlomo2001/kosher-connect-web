...
// ── The matrix ───────────────────────────────────────────────────────────
// requirement(destination, nationality) → an AUTH entry, plus an optional
// per-cell note where reality is nuanced. Home country → NONE.
const NUANCE = {
...
function cell(dest, nat) {
  switch (dest) {
    case 'US':
      if (nat === 'US') return AUTH.NONE
      if (nat === 'CA') return AUTH.NONE // Canadians: no ESTA, no visa for tourism
      if (['GB','IL','BE','CH','HU','AT'].includes(nat)) return AUTH.ESTA
      return AUTH.CHECK
    case 'CA':
      if (nat === 'CA') return AUTH.NONE
      if (nat === 'US') return AUTH.NONE // US citizens are exempt from the eTA
      if (nat === 'IL') return AUTH.CHECK // visa vs eTA — genuinely conditional
      if (['GB','BE','CH','HU','AT'].includes(nat)) return AUTH.ETA_CA
      return AUTH.CHECK
    case 'IL':
      if (nat === 'IL') return AUTH.NONE
      if (['GB','US','CA','BE','CH','HU','AT'].includes(nat)) return AUTH.ETA_IL
      return AUTH.CHECK
    case 'EU':
      if (['BE','CH','HU','AT'].includes(nat)) return AUTH.NONE // EU/EFTA freedom of movement
      if (['GB','US','CA','IL'].includes(nat)) return AUTH.NONE // + ETIAS-coming note
      return AUTH.CHECK
    case 'UK':
      if (nat === 'GB') return AUTH.NONE
      if (['US','CA','IL','BE','CH','HU','AT'].includes(nat)) return AUTH.ETA_UK
      return AUTH.CHECK
    default:
      return AUTH.CHECK
  }
...
// Public: what does this passenger need for this trip?
export function requirementFor({ destination, nationality }) {
  ...
  const auth = cell(dest, nat)
  const note = NUANCE[`${dest}:${nat}`] || null
...
// for ~6 months beyond travel. Returns ok=null when we don't hold an expiry.
export function passportCheck({ passportExpiry, travelDate, monthsRequired = 6 }) {
  if (!passportExpiry) return { ok: null, note: 'No passport expiry on file.' }
  const exp = new Date(passportExpiry + 'T00:00:00Z')
  const travel = travelDate ? new Date(travelDate + 'T00:00:00Z') : null
  if (Number.isNaN(exp.getTime())) return { ok: null, note: 'Passport expiry not readable.' }
  // The date the passport must still be valid *to*: travel + required months.
  ...
  return {
    ...
    note: ok
      ? null
      : `Passport should be valid until at least ${needBy.toISOString().slice(0, 10)} (≈${monthsRequired} months after travel).`,
  }
}
...
// not-needed|check.
export function coverageStatus({ requirement, travelDate, auths = [], warnDays = 30 }) {
  ...
  if (travel && !Number.isNaN(travel.getTime()) && validUntil.getTime() < travel.getTime()) {
    return { status: 'expiring', validUntil: match.valid_until, expiresBeforeTravel: true }
  }
