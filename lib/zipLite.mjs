...
export async function unzip(input) {
  ...
  for (let n = 0; n < count; n++) {
    ...
    const expectedCrc = dv.getUint32(off + 16, true)
    ...
    entries.push({
      ...
      bytes: async () => {
        ...
        // Reject a member whose bytes don't match the central-directory CRC-32
        // rather than feeding corrupt data into the contacts/messages importer.
        // The central dir always holds the real CRC (written after the payload),
        // so this is correct even for data-descriptor members. A 0 CRC means the
        // writer didn't record one (e.g. some streamed zips) — skip the check.
        if (expectedCrc !== 0 && crc32(out) !== expectedCrc) {
          throw new Error(`Corrupt ZIP member ${name} — CRC-32 mismatch.`)
        }
        return out
      },
    })
