...
export function matchSimForMail(mail, index) {
  ...
  if (candidates.length > 1) {
    ...
    // Narrowed to NOTHING is not the same as narrowed to several. Lebara sends
    // account-level mail to the BARE base address — 336 SIMs share it — while
    // the SIM the mail is about is registered under its +tag, so it is not in
    // the address-candidate set at all. The mail still names its number in the
    // body; a number that matches exactly ONE SIM on the books identifies it
    // regardless of what the envelope matched. This was the whole gitt.bilig
    // pile-up (owner, 23 Aug: "a lot of carrier emails just to gittbilig — i
    // have no use from this!"): 90+ renewal and payment notices, every one
    // carrying its number, parked as ambiguous. Several-narrowed stays
    // ambiguous — there the number genuinely failed to decide.
