...
export const SCREEN_ACTIONS = {
  ...
  // Without an entry here the inbox fell back to "Nothing waiting on you here"
  // while a Waiting row sat underneath it — the row and the list contradicting
  // each other on one screen, which is the exact failure this module exists to
  // prevent. The button still earns its place on the screen that owns the work:
  // from All it narrows the list to the ones waiting.
  messages(f) {
