...
export function phoneProblem(raw) {
  ...
  // The country code that got dropped. A UK mobile is 07 + 9 digits, so typing
  // it with the leading 0 replaced by + — or pasting it without the 44 —
  // produces a 10-digit number starting 7. That cannot be a real international
  // number either: country code 7 (Russia, Kazakhstan) always carries 11
  // digits, so a 10-digit one beginning 7 reaches no handset anywhere.
  //
  // Narrow on purpose. It fires only on that exact shape, so a correctly typed
  // 07…, +447… or any genuine foreign number is untouched. Added 25 Aug after
  // the first live test SMS went to Twilio and came back rejected — this guard
  // passed it as "10 digits, plausible" and the provider knew better.
