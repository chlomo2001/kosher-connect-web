...
//
// Two Excel hazards have to be handled BEFORE counting digits, because both
// silently produce a plausible-looking wrong identifier:
//
//  • A numeric cell arrives as "359370792455705.0". Stripping non-digits turns
//    the decimal zero into a real one — a 15-digit IMEI becomes a 16-digit
//    number that belongs to nobody. Drop the fractional part first.
//  • A long numeric cell can arrive in scientific notation (8.9012E+19). The
//    digits are simply gone; there is nothing to recover, so it is flagged
//    rather than turned into a short fake ID.
export function parseIdField(raw, { min, max }) {
  const v = str(raw)
  if (!v) return { value: '', leftover: '', clear: true }
  if (/e\+?\d/i.test(v)) return { value: '', leftover: v, clear: false }
  const d = digits(v.replace(/\.\d+$/, ''))
...
export function parsePoolDetails(raw) {
  ...
  const stray = parseIdField(v, { min: 14, max: 16 })   // same Excel-number care
  if (stray.clear && stray.value) out.imei = stray.value
