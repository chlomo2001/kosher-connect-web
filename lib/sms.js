...
import { normalisePhoneE164, phoneProblem } from './phoneNumber.mjs'
...
export async function sendSms({ to, body, customerId, repliesTo = null }) {
  if (!smsEnabled) throw new Error('SMS not configured')
  const toNorm = normalisePhoneE164(to)
  // `repliesTo` rides on EVERY outcome below, the failures included. A reply
  // that was held, redirected or refused still answers a particular question,
  // and the log should say which — lib/replyQueue.mjs decides separately
  // whether the outcome counts as the customer having been answered, and it
  // only counts 'sent'. Recording the link on failures is what lets the queue
  // show "tried, did not go" rather than losing the attempt entirely.
  const base = {
    to_email: toNorm,
  ...
  // Check the DESTINATION before the gate, not after. In TEST mode every
  // message is redirected to the shop's own handset, so a number that could
  // never receive anything still comes back "sent" and the log agrees — which
  // is exactly how a too-long number got through unremarked (owner, 08-17).
  // Refused here, it is written to the same log as everything else, so the
  // number that could not be texted is visible rather than merely absent.
  const problem = phoneProblem(to)
  if (problem) {
    await logSend({ ...base, status: 'invalid', error: `${problem.code}: ${problem.message}`.slice(0, 500) })
