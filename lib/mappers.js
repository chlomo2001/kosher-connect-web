...
export function phoneToRow(p) {
  return {
    ...
    //
    // …EXCEPT addedAt, which listPhones reads off `created_at` and merges on.
    // Letting it ride back in would put a copy of a column inside the blob, and
    // the two would answer "when did this arrive" separately from the first
    // time either changed. Same rule, same reason, as lib/provenance.mjs.
    legacy_extras: stripDerived(p),
  }
}

/** The read-side fields listPhones adds, removed again before storing. */
const stripDerived = (p) => {
  if (!p || typeof p !== 'object' || !('addedAt' in p)) return p
  const out = { ...p }
  delete out.addedAt
  return out
}
