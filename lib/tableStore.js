...
export async function syncRentals(appRentals) {
  ...
  // IN the payload — keep their previously-synced rows untouched). Money
  // history survives: reversed to zero, then detached (FK set-null).
  const deleted = await deleteMissingRentals(appRentals.map((r) => r.id))
...
function bucketTargets(r) {
  ...
  return {
    charge: -money(r.price),
    late: returned ? -money(r.lateFee) : null, // null = leave whatever is posted
...
// Zero a rental's ledger position, then delete the row; the FK detaches the
// history (related_rental_id → NULL) so the customer keeps every entry.
async function deleteMissingRentals(keptLegacyIds) {
  const existing = await db.select('rentals', 'select=id,legacy_id&legacy_id=not.is.null')
  const keep = new Set(keptLegacyIds.map(String))
  const gone = existing.filter((r) => !keep.has(String(r.legacy_id)))
  for (const r of gone) {
    const entries = await db.select('ledger', `select=amount,customer_id&related_rental_id=eq.${r.id}`)
    ...
    if (entries.length && Math.abs(net) >= 0.005) {
      await db.insertIgnoreDup('ledger', [{
        ...
        charge_reference: `RENTAL-REVERSAL-${r.id}`,
    ...
    await db.delete('rentals', `id=eq.${r.id}`)
  }
  return gone.length
}
