 ...
 */
/**
 * How much a set of agreeing signals is worth. Stated ONCE, because the API
 * scan and this module both need it and two copies of a judgement rule drift.
 *
 * Signals are named: 'name', 'email', 'phone'.
 *
 *   'high'   — two or more independent signals agree.
 *   'medium' — one contact signal on its own. A shared phone is NOT proof
 *              here: one family phone serving several real customers is
 *              everyday reality at the counter, and mailboxes are shared the
 *              same way. Medium means "look", never "merge".
 *   'low'    — the written name alone. Same-name strangers are routine in a
 *              community drawing on one pool of names.
 */
export function duplicateConfidence(signals = []) {
  const kinds = new Set([...signals].filter(Boolean))
  if (kinds.size >= 2) return 'high'
  return kinds.has('name') || kinds.size === 0 ? 'low' : 'medium'
}

/** signalKey → [customer ids]. `n:` name, `e:` person email, `p:` phone. */
function bucketise(customers) {
...
/**
 * The contact details too many people share to be a duplicate signal.
 *
 * A number twenty people share is the SHOP's number, or a school office, or the
 * phone in a hall — not evidence that twenty people are one person. Left in, it
 * is also what would drown the queue: one bucket of fifty is 1,225 pairs, all
 * wrong, filling a screen meant for judgement. findDuplicates drops them; this
 * says which, because a signal that has silently stopped applying is worse than
 * one that says so out loud.
 *
 * Names are never capped — a common name IS the low-confidence signal, and
 * dropping the commonest ones would blind the scan to exactly the community
 * where a pool of shared names is the whole problem.
 */
export function sharedContacts(customers = [], { maxBucket = 8 } = {}) {
  const out = []
  for (const [key, ids] of bucketise(customers)) {
    const kind = key[0]
    if (kind === 'n' || ids.length <= maxBucket) continue
    out.push({ kind: kind === 'p' ? 'phone' : 'email', value: key.slice(2), count: ids.length, ids })
  }
  return out.sort((a, b) => b.count - a.count)
}

export function findDuplicates(customers = [], { maxBucket = 8 } = {}) {
  const buckets = bucketise(customers)

  const pairSignals = new Map()   // "a|b" (sorted) → Set of signal kinds
  for (const [key, ids] of buckets) {
    if (ids.length < 2) continue
    const kind = key[0]           // n / e / p
    // Too many people on one contact detail means it is shared, not duplicated
    // — see sharedContacts, which reports exactly what was dropped here.
    if (kind !== 'n' && ids.length > maxBucket) continue
