...
export function parseSmsBackupXml(text) {
  ...
  // The MESSAGES are the test, not the wrapper. This used to demand an <smses>
  // root, which is what SMS Backup & Restore writes — but the owner's own
  // xml→fig converter never checked for it, so files that tool happily ate got
  // refused here and the job went back to the office PC. Anything carrying
  // <sms …> elements is worth reading; a file with none of them is the real
  // error, and still says so.
  if (!/<sms\b/.test(src)) {
    throw new Error('No <sms> entries in this XML — expected an SMS Backup & Restore export.')
  }
  const out = []
  // Match the whole <sms …> tag, but let a '>' INSIDE a quoted attribute value
  // survive: XML doesn't require escaping '>' in attribute values, so a message
  // body like "u there? ->" carries a raw '>'. The old /[^>]*?/ stopped at that
  // first '>', truncating the tag and silently dropping the message (and often
  // corrupting the scan position). Consume either a full "quoted span" (any '>'
  // inside is safe) or a single non-'>' non-'"' char; only an UNquoted '>' ends
  // the tag.
  // Single quotes are legal XML too, and reading only double-quoted attributes
  // was worse than dropping the message: every attribute missed, so the record
  // came out BLANK and an empty text went into the backup. Both quote styles,
  // and the closing quote has to match the opening one or a body containing an
  // apostrophe would end its own attribute.
  const smsRe = /<sms\b((?:"[^"]*"|'[^']*'|[^>"'])*)\/?>/g
  const attrRe = /([\w:]+)=("([^"]*)"|'([^']*)')/g
  let m
  while ((m = smsRe.exec(src)) !== null) {
    ...
    while ((am = attrRe.exec(m[1])) !== null) {
      a[am[1]] = decodeEnt(am[3] !== undefined ? am[3] : am[4])
    }
    // Nothing to carry is not a message. Taken from the owner's own converter,
    // which guards with `if (attrs.address || attrs.body)` — without it a tag
    // we failed to read becomes a blank line in the restore.
    if (!a.address && !a.body) continue
    out.push({
      address: a.address || '',
      body: a.body || '',
      date: a.date || '0',
