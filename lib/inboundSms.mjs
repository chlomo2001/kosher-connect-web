...
export function inboundLogRow({ from, to, body, messageSid, customerId = null }) {
  ...
  // customer_id is a uuid column, and the FIRST real reply (18 Aug) was lost
  // to a legacy_id being filed there — the insert 400ed and took the whole
  // message with it. A wrong-shaped id is dropped instead: a reply filed
  // unmatched is readable and fixable; a reply that never lands is gone.
  const uuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(String(customerId || ''))
  return {
    provider: 'twilio',
    kind: 'sms_in',
    ...
    customer_id: uuid ? customerId : null,
  }
}
...
/**
 * Can this log row be replied to, and where does the reply go?
 *
 * Pure on purpose: the endpoint fetches the row, this decides. The decision is
 * the part worth testing, and the part that must not quietly change.
 *
 * Returns { phone, customerId, original } or { error } — a plain sentence for
 * staff, never a code.
 */
export function replyTarget(row) {
  if (!row) return { error: 'That message is no longer in the log.' }
  if (row.provider !== 'twilio' || row.kind !== 'sms_in') {
    return { error: 'You can only reply to a text that came in.' }
  }
  // The one message this shop must not send. Twilio blocks it at their end
  // too, but a control that offers to do it and then fails has already made
  // the promise — refuse it here, in words.
  if (row.status === 'opt_out') {
    return { error: 'They texted STOP. Replying would be a message they asked not to get.' }
  }
  const phone = String(row.to_email || '').trim()
  if (!phone) return { error: 'That message has no number to reply to.' }
  return {
    // The id of the row actually checked, so a caller records the link against
    // what it verified rather than against whatever the browser sent it.
    id: row.id,
    phone,
    // Only a uuid may go back into the uuid column — the same rule the inbound
    // writer follows, and for the same reason it was learned.
    customerId: /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(String(row.customer_id || ''))
      ? row.customer_id : null,
    original: String(row.subject || ''),
  }
}
