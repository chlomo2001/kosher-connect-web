...
export function providersForCountry(country, used = []) {
  ...
  // No country given — the SIM-plan form has none, because SIM plans are the UK
  // SIM-only side of the shop and were never split by country. There it offers
  // everything rather than pretending to a scope it does not have.
  if (country) (PROVIDER_SEEDS[country] || []).forEach(add)
  else Object.values(PROVIDER_SEEDS).flat().forEach(add)
  // A used name only joins a COUNTRY's list if it was used in that country.
  // Entries with no country recorded — every SIM plan, which has no country
  // field — belong in the unscoped list and nowhere else. Letting them through
  // put Lebara and 1pMobile in the USA dropdown, which is precisely the hunt
