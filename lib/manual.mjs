...
export const SCREENS = [
  // ── The frame around every screen ──────────────────────────────────────
  {
    ...
    parts: [
      ...
      ['Hide the counts', 'Every screen with a row of counts along the top carries this, small and grey above the cards on the right. Pressing it folds the cards away so the list gets the screen \u2014 and the button becomes the figures, reading them along one line, so nothing is put away without still being said. Pressing it again brings the cards back; the button never goes, so there is always the way back. It remembers per screen, so the ones you work in as a list stay folded and the ones you come to for the numbers stay open. Two things never fold: the controls \u2014 buttons, view pills, filters, anything you press \u2014 and the line under the title when it is asking for something. "3 phones overdue back" stays whatever the fold says; only "nothing waiting on you here" folds, because that is the one with nothing to say.'],
  ...
  },
  {
    ...
    parts: [
      ...
      ['Phone Inventory', 'The handsets themselves, not the hires: what each one is, whose it is, and whether it is out, free or retired. Opening one shows its History at the bottom, folded: the day it was added here, and every hire it has been out on, newest first, each one opening the hire itself. The summary line says it without unfolding — how many hires, and how long the shop has had it. Two honesties are built into it. For a handset that arrived in an import the arrival date is the day the APP learned about it, not the day it was bought, and the line says so rather than making a claim the record cannot support. And where a hire is older than the arrival date — which happens, because handsets were imported while hires were already running — the arrival line explains that too, instead of leaving a trail that looks like a mistake.'],
      ...
      ['Availability', 'The calendar view — which phones are committed on which days. Use it before promising a phone for a date. It starts at today rather than at the 1st, because a day already gone is not availability; when what is left of the month is too short to plan in, the columns run on into the next one. “Whole month” puts the days already gone back, for the rarer question of what the fleet did this month. Stepping back a month shows that month entire — looking at what happened is a different question from what is free.'],
  ...
  },
  {
    ...
    parts: [
      ...
      ['Needs a human', 'The queue: messages the app could not confidently file. This is the whole job of the screen. A long queue arrives a page at a time \u2014 the card above says how many are waiting altogether, and a line under the list says how many of those are on screen, with a button for the next lot. The tick-all above the list ticks the page you are looking at, and says so.'],
  ...
  {
    id: 'messages', kind: 'staff', name: 'Messages', status: 'written',
    what: 'The texts customers have sent the shop, as conversations — and the place they get answered.',
    parts: [
    ...
    rules: [
      'A message is answered only once a reply has actually REACHED them. While texting is on hold, or pointed at the test number, the reply is written and logged and the customer receives nothing. Nor does a reply the network refused count: the carrier reports back a few seconds after sending, and a reply it could not deliver leaves the person in the waiting list where they belong. This is the one place the screen deliberately disagrees with what you just did, and it is right to. What it will not do is guess: while a network has not reported back yet, the reply is taken at face value, because treating silence as failure would leave every answer in the queue for ever.',
    ...
    wrong: [
      ...
      ['The reply said it was sent but they never got it', 'Read the bubble. HELD or TEST means it never left the building — texting is not live, or it is pointed at the shop\'s own test number, and Settings → Messaging says which. "Did not arrive" means the opposite: it went, and the network could not deliver it. Hover it for the carrier\'s reason — usually a handset switched off, a number that no longer exists, or a landline.'],
  ...
  },
  {
    ...
    parts: [
      ...
      ['Communications', 'The card is called Messaging (email & SMS) on the screen — email and SMS, each with its safety gate, the addresses post arrives at, a test send for each channel, and the log of every message: sent, and the replies customers text back, marked ↩ so the two directions read apart. A text that came in carries a Reply button, and one nobody has answered says so on its row. The log does not load itself; press Load the log. Answering one happens on Messages, not here: the Reply button on a row takes you to that conversation, because a question is answered in the context of the ones around it. This screen is the audit trail — what went out, on every channel — and Messages is the inbox. The two test sends prove the connections without touching a customer: type your own address or number and press send. Read the line beside each one first — while a channel is live it really sends to whatever you type, and the line says so. Below the two channels sits the morning digest: everything still waiting, in one email at half past six each morning. It has its own row saying whether anyone is actually receiving it, because a job that runs every day and quietly posts to nobody looks exactly like a job that is working.'],
      ...
      ['Automation', 'The jobs the nightly sweep runs for you, and what each one is allowed to do. A rule watches for something — a debt over a figure, a flight coming up, a passport running out, a SIM about to renew — and either raises a task for you or, for two of them only, texts the customer itself. Passport and SIM renewal are the two, because the fact is plain and the customer is glad to be told; everything else, money especially, raises a task so a person decides. A rule that texts never texts the same person about the same thing twice, and never on Shabbos or yom tov. It also stays switched off until the shop deliberately turns it on: until then the sweep writes down what it would have said, so it can be read before anyone receives it.'],
    ...
    wrong: [
      ...
      ['An automation says it would have texted somebody, but nobody got a text', 'That is it working as intended. A rule that texts is held shut until the shop switches automatic texting on; until then it does everything except send, and reports what it would have said. The point is that you can read a week of it before a customer ever does.'],
      ['The morning email never arrives', 'Look at the morning digest row under Messaging. If it does not say ON, nobody is set as the recipient and the job has been running every morning sending to nobody — the row says what to fill in. If it does say ON and a morning still passes with no email, that is the digest staying quiet on purpose: it sends nothing at all on a morning with nothing open.'],
...
/**
 * A short, stable fingerprint of the manual's CONTENT — it changes exactly
 * when a screen's prose changes, and never otherwise.
 *
 * From the Virtual Mail navigation map (owner shared it 21 Aug): every one of
 * its 33 pages carries "v7.88.0 • 2026-08-21", so a printed copy lying by the
 * till can be checked against the live one at a glance. KC's manual is built to
 * be printed — and a printout had no way at all to say how old it was.
 */
export function manualFingerprint(screens = SCREENS) {
  const src = JSON.stringify(screens)
  let h = 5381
...
/**
 * The stamp the page and the printout carry. MAINTAINED BY HAND, HELD BY TEST:
 * test/manual.test.mjs fails the moment `fingerprint` stops matching
 * manualFingerprint(SCREENS), so a content change cannot ship without moving
 * `date` forward in the same commit — the same way the manual itself cannot
 * fall behind the screens. The date is therefore "when the words last
 * changed", which is the only date a reader of a printout actually wants.
 */
export const MANUAL_STAMP = { date: '2026-08-30', fingerprint: 'm-5211d3a0' }

/** "As of 21 Aug 2026 · m-xxxxxxxx" — one line, same everywhere. */
export function manualStampLine() {
  const [y, m, d] = MANUAL_STAMP.date.split('-').map(Number)
  const MON = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']
  return `As of ${d} ${MON[m - 1]} ${y} · ${MANUAL_STAMP.fingerprint}`
}
