...
export const SCREENS = [
  ...
  },
  {
    ...
    parts: [
      ...
      ['Communications', 'The card is called Messaging (email & SMS) on the screen — email and SMS, each with its safety gate, the addresses post arrives at, a test send for each channel, and the log of every message: sent, and the replies customers text back, marked ↩ so the two directions read apart. A text that came in carries a Reply button, and one nobody has answered says so on its row. The log does not load itself; press Load the log. Answering one happens on Messages, not here: the Reply button on a row takes you to that conversation, because a question is answered in the context of the ones around it. This screen is the audit trail — what went out, on every channel — and Messages is the inbox. The two test sends prove the connections without touching a customer: type your own address or number and press send. Read the line beside each one first — while a channel is live it really sends to whatever you type, and the line says so.'],
 ...
 */
export const MANUAL_STAMP = { date: '2026-08-25', fingerprint: 'm-824cf0c9' }
