...
export function formatPhoneDisplay(raw) {
  ...
  if (!s) return ''
  if (/[A-Za-z]/.test(s)) return s                       // alphanumeric sender
  let n = normalizeUkNumber(s)
  // Bare international digits, DISPLAY ONLY. normalizeUkNumber deliberately
  // leaves these alone — it also keys customer matching, and guessing a
  // country onto an ambiguous number there would merge two real people. But
  // the rental inventory stores its handsets as bare digits ("17185998801"),
  // so every US/Canada/Israel phone rendered raw: no +, no grouping, on the
  // availability calendar and everywhere else the number is shown.
  // Only an explicit, unambiguous country code with the right digit count is
  // promoted; a bare 10-digit number stays untouched because it could be
  // either a US number missing its 1 or a national number of somewhere else.
  if (!n.startsWith('+') && !/[A-Za-z]/.test(n)) {
    const d = n.replace(/\D/g, '')
    if (/^1\d{10}$/.test(d)) n = '+' + d                  // +1 and 10 national digits
    else if (/^972\d{8,9}$/.test(d)) n = '+' + d          // +972, landline or mobile
  }
