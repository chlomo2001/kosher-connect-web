...
const AIRLINES = [
  ...
  // body rather than the sender. Owner, 18 Aug: "sometimes its booked via 3rd
  // party like wiki.com and much more". THIS LIST WILL NEVER BE COMPLETE, and
  // it does not have to be: an agent nobody has heard of still gets through on
  // the ticket phrases below. All the list decides is what NAME goes on the
  // booking before a person confirms it.
  [/edreams/i, 'eDreams'],
  [/opodo/i, 'Opodo'],
  [/kiwi\.com/i, 'Kiwi.com'],
  [/expedia/i, 'Expedia'],
  ...
  [/esky/i, 'eSky'],
  [/travelup/i, 'TravelUp'],
  [/alternativeairlines/i, 'Alternative Airlines'],
  [/budgetair/i, 'BudgetAir'],
  [/netflights/i, 'Netflights'],
  [/ebookers/i, 'ebookers'],
  [/cheapoair/i, 'CheapOair'],
  [/skyscanner/i, 'Skyscanner'],
  [/kayak/i, 'Kayak'],
  [/momondo/i, 'Momondo'],
  [/flightnetwork/i, 'Flight Network'],
...
// An unpaid reservation is the one airline email that costs money to ignore:
// the seats are held, nobody has paid, and the airline cancels it quietly.
const PAYMENT_DUE = /\b(complete\s+(the\s+)?payment|payment\s+(is\s+)?(due|required|outstanding|pending)|unpaid|not\s+yet\s+paid|awaiting\s+payment)\b/i
const CHECKIN = /\b(check[-\s]?in\s+(is\s+)?(now\s+)?open|online\s+check[-\s]?in|check\s+in\s+for\s+your\s+flight)\b/i
// Evidence that a message is about a REAL booking rather than about the airline.
// Broad on PURPOSE: a genuine confirmation is the thing that must never be
// missed, and marketing is already stopped by NOT_A_BOOKING on the subject
// before this is ever consulted. A narrow version dropped a real Wizz Air
// confirmation — "Confirmation number: XU2WWH", "Your booking is confirmed",
// flight "W6 5623" — because none of them said the exact words "booking
// reference". These are the phrases a confirmation actually uses.
const BOOKING_EVIDENCE = [
  /\b(itinerary|e-?ticket|boarding\s+pass|reservation|PNR)\b/i,
  /\bbooking\s+(reference|confirmation|is\s+confirmed|confirmed|number)\b/i,
  /\bconfirmation\s+(number|code|reference)\b/i,
  /\b(reference|reservation|confirmation)\s*(number|no\.?|code)?\s*[:#]/i,
  /\bdeparture\b\s*[:\-]/i,               // "Departure: 12 September"
  /\b[A-Z]{3}\s*(?:→|->|to)\s*[A-Z]{3}\b/,   // LTN → TLV
  /\bflight\b.{0,30}\b[A-Z]{1,2}\d{2,4}\b/is, // "flight W6 5623"
]
// Mail an airline sends everyone. It arrives from the same address as a real
// confirmation, which is why the sender alone can never be the test: on 18 Aug
// the ticket queue held three copies of "We're updating our Privacy Policy",
// two baggage notices and an advert for bag drop — each dressed as a booking,
// and the privacy policy had a TRAVEL DATE parsed out of it.
const NOT_A_BOOKING = [
  /privacy\s+policy/i, /terms\s+(and|&)\s+conditions/i, /newsletter/i, /unsubscribe/i,
  /\b(deals?|offers?|sale|discount|promo)\b/i, /countdown/i, /bag\s+drop/i,
  /baggage\s+is\s+on\s+board/i, /\b(survey|rate\s+your|tell\s+us\s+what)\b/i,
  /forwarding\s+confirmation/i, /gift\s+card/i, /\bwin\b/i, /\bcompetition\b/i,
]
...
export function looksLikeTicket({ from = '', subject = '', text = '' } = {}) {
  ...
  // Nothing an airline sends everyone is a ticket, whoever it came from. This
  // is checked FIRST and it wins: 8 of the 10 messages the queue had filed by
  // 18 Aug 2026 were circulars from real airline addresses.
  if (NOT_A_BOOKING.some((re) => re.test(subj))) return false
  const body = `${subj}\n${String(text).slice(0, 4000)}`
  // An airline sender is a HINT, not a verdict — it still has to be about a
  // booking. It was a verdict until today, which is how "We're updating our
  // Privacy Policy" became a flight with a travel date of 14 September.
  if (airlineOf({ from })) return BOOKING_EVIDENCE.some((re) => re.test(body))
...
/**
 * What an airline email IS. 'other' is the default and it is deliberate.
 *
 * Until 18 Aug 2026 the default was 'confirmation', so every message the queue
 * could not read asserted itself to be a booking: a privacy-policy notice, a
 * baggage advert and a Gmail forwarding receipt all sat in the register under
 * "✈️ Confirm booking details". A default that names a meaning is a guess
 * wearing the clothes of a fact — the carrier-mail queue says 'other' when it
 * does not know, and so does this one now.
 */
export function ticketKind({ subject = '', text = '' } = {}) {
  const subj = String(subject || '')
  const head = `${subj}\n${String(text).slice(0, 1200)}`
  // Marketing is decided on the SUBJECT. An airline's advert quotes real
  // booking words further down ("your booking", a reference in the footer), so
  // reading the body first would dress every circular as a confirmation.
  if (NOT_A_BOOKING.some((re) => re.test(subj))) return 'marketing'
  if (CANCELLED.test(head)) return 'cancellation'
  if (PAYMENT_DUE.test(head)) return 'payment_due'
  if (CHANGED.test(head)) return 'change'
  if (CHECKIN.test(head)) return 'checkin_open'
  if (BOOKING_EVIDENCE.some((re) => re.test(head))) return 'confirmation'
  return 'other'
}

/** How each kind reads on screen. 'other' says nothing rather than guessing. */
export function ticketKindLabel(kind) {
  return ({
    cancellation: 'Cancelled',
    change: 'Flight changed',
    payment_due: 'Not paid yet',
    checkin_open: 'Check-in open',
    confirmation: 'Booking confirmation',
    marketing: 'Not about a booking',
    other: '',
  })[kind] || ''
}

/** The kinds that are not a booking to make, and should not offer to become one. */
export const NOT_BOOKABLE = new Set(['marketing', 'other'])
...
// ── is this one already on the books? ─────────────────────────────────────
//
// The same confirmation is often received twice — the airline resends it, the
// customer forwards it after the shop already booked it, a change notice
// repeats the original reference. Until 18 Aug 2026 every copy offered "Confirm
// booking details", which opens a NEW booking form: press it and the shop has
// the same journey twice, charged twice, and two rows to reconcile against one
// payment. Owner: "a pnr which is already an active booking in system, it
// shouldnt open a new booking - obviously".

/** A PNR as a comparable key. Airlines print them with spaces, dashes and in
 *  either case; none of that is part of the reference. */
export function refKey(ref) {
  return String(ref || '').toUpperCase().replace(/[^A-Z0-9]/g, '')
}

/**
 * The live booking that already carries this reference, or null.
 *
 * Legs count: a self-transfer journey holds several references, one per
 * airline, and the second email is a leg of the first booking rather than a
 * booking of its own.
 *
 * A CANCELLED booking does not count. Its reference is spent — a rebooking can
 * legitimately reuse it, and blocking that would leave the counter unable to
 * enter the replacement.
 */
export function bookingForReference(reference, bookings = []) {
  const key = refKey(reference)
  // Five is the shortest reference any airline the shop deals with issues.
  // Below that a mis-parse ("BA", "OK") would match half the register.
