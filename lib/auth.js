...
import { emailsMatchLoose } from './mappers.js'
...
/**
 * Is this signed-in user allowed to BE somebody in the portal?
 *
 * Pure so it can be tested without a network: the decision is the part that
 * matters. The portal binds a signed-in email to a customer record and then
 * shows that customer's balance, rentals, flights, SIM plans and ledger — so
 * the binding is only ever as good as the claim that this person owns the
 * mailbox. An unconfirmed address is not that claim.
 *
 * Supabase already withholds a session until an email is confirmed, so today
 * this is a second lock on a door that is shut. It is worth having because the
 * first lock is a dashboard setting: turn "Confirm email" off, or add a
 * provider that reports an address it has not verified, and the portal would
 * hand a stranger somebody else's money without a line of code changing.
 *
 * `confirmed_at` is Supabase's generated column (email OR phone); both are
 * checked so a phone-confirmed account is not locked out by accident.
 */
export function portalUserIsUsable(user) {
...
export async function verifyPortalToken(accessToken) {
  ...
  const user = r && r.ok && r.json ? r.json : null
  return portalUserIsUsable(user) ? user : null
}
...
// Best-effort in-memory brute-force guards (same accepted trade-off as
// staffCache: serverless recycling only makes it stricter, and Supabase's OTP
// rate limit is the durable backstop).
//
// Both counters key on the EMAIL, not the ticket: a ticket-keyed 2FA counter
// reset every time the attacker minted a fresh ticket with the password, so
// the limit never bound (sweep 2026-08-02 #6). Window-based, so a legitimate
// fumbled sign-in recovers on its own instead of needing a redeploy.
const FAIL_WINDOW_MS = 15 * 60 * 1000

function failEntry(map, email) {
  const key = String(email || '').trim().toLowerCase()
  const e = map.get(key)
  if (e && Date.now() - e.first > FAIL_WINDOW_MS) map.delete(key)
  return map.get(key)
}
function noteFail(map, email) {
  const key = String(email || '').trim().toLowerCase()
  const e = failEntry(map, key) || { n: 0, first: Date.now() }
  e.n += 1
  if (map.size > 500) map.clear()
  map.set(key, e)
}
const clearFails = (map, email) => map.delete(String(email || '').trim().toLowerCase())

// 2FA: void the second factor after too many wrong codes, so the 6-digit
// space can't be ground even with the password in hand. audit C13.
const twofaFails = new Map()
const MAX_2FA_ATTEMPTS = 5
export function twofaLocked(email) {
  const e = failEntry(twofaFails, email)
  return !!(e && e.n >= MAX_2FA_ATTEMPTS)
}
export function note2faFailure(email) { noteFail(twofaFails, email) }
export function clear2faFailures(email) { clearFails(twofaFails, email) }

// Password step: /api/auth/login answers as an oracle (401 wrong / 200 right),
// so unlimited guessing must not be free (sweep 2026-08-02 #5). Complements
// the per-IP rate limit in the route itself.
const loginFails = new Map()
const MAX_LOGIN_ATTEMPTS = 8
export function loginLocked(email) {
  const e = failEntry(loginFails, email)
  return !!(e && e.n >= MAX_LOGIN_ATTEMPTS)
}
export function noteLoginFailure(email) { noteFail(loginFails, email) }
export function clearLoginFailures(email) { clearFails(loginFails, email) }
...
// Every destination the app has. This is not documentation — the owner's
// allowedTabs IS this list, and main.js hides any .nav-item whose data-tab is
// missing from it. A tab added to NAV and TAB_META but not to this array is
// invisible in production to everyone, owner included, while looking perfectly
// fine in the offline harness (which runs with auth off, so allowedTabs is
// null and nothing is filtered). 'review' shipped that way on 08-16.
// test/tabs.test.mjs now holds the three lists together.
export const ALL_TABS = ['dashboard', 'customers', 'messages', 'rentals', 'sim', 'wallet',
  'bookings', 'repairs', 'services', 'shop', 'koltorah', 'virtual', 'tasks',
...
export async function helperTabs() {
  ...
  // Fail CLOSED (sweep 2026-08-02 #9): the old "empty/broken row returns all
  // tabs" meant a bad restore silently promoted every helper to full access.
  // Failing closed is not a lock-out — reads (GETs) never consult this and
  // owners always pass — it just stops helper WRITES until the owner re-saves
  // the Team setting, which is the right failure for an access-control list.
  if (!list.length) {
    console.warn('[auth] helper_tabs setting missing or empty — helper writes disabled until it is re-saved')
  }
  return list
}
...
export async function staffProfileByEmail(email) {
  ...
  const rows = await db.select('staff_profiles',
    `select=id,role,full_name,email&email=eq.${encodeURIComponent(target)}`)
  if (rows.length) return rows[0]
  // Gmail treats dotted/undotted locals as one mailbox, so Google can report
  // a different spelling than the register holds. Staff list is small —
  // compare loosely in memory rather than guessing spellings in SQL.
  const all = await db.select('staff_profiles', 'select=id,role,full_name,email&email=not.is.null&limit=100')
  return all.find((r) => emailsMatchLoose(r.email, target)) || null
}

// Find an existing auth user by email (gmail-dot tolerant). Pages through the
// admin list — fine at this scale; returns null past ~2000 users rather than
// scanning unboundedly.
export async function adminFindUserByEmail(email) {
  for (let page = 1; page <= 10; page++) {
    const r = await authRequest(`admin/users?page=${page}&per_page=200`)
    if (!r.ok) return null
    const users = r.json?.users || []
    const hit = users.find((u) => emailsMatchLoose(u.email, email))
    if (hit) return hit
