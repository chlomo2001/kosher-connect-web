...
// Staff-facing wording for a failed database write. "Storage error" named the
// component; this names the consequence and the next step, which is what the
// person at the counter actually needs. The real error still goes to
// console.error / the server log.
export const STORAGE_ERROR =
  'Couldn’t save that — nothing was changed. Try again, and if it keeps happening tell the owner.'
...
async function rest(pathAndQuery, { method = 'GET', body, prefer } = {}) {
  ...
  // Bound every DB round-trip: a hung upstream (Supabase incident, network
  // blackhole) must fail fast and throw the normal '[db] …' error, not block the
  // whole serverless function — /api/health can't answer 503 quickly otherwise.
...
// PostgREST builds ONE column list per POST, so every row in a batch must
// carry exactly the same keys — a mixed batch (e.g. a shop charge row without
// `method` next to its settlement payment row with it) is rejected whole with
// PGRST102 "All object keys must match", which took the till down in
// production. Level the batch: fill each row's missing keys with explicit
// nulls, which is what a uniform caller would have written by hand. (A row
// omitting a defaulted column gets null instead of the default — acceptable,
// because before this the same batch didn't insert at all.)
export function uniformRows(rows) {
...
// cap — an unbounded SELECT silently stops at the cap (this bit the ledger
// report once, and the sims table is at 840 rows and climbing). `tail` is
// everything after the column list (filters + a stable order); pages walk
// until a short page proves the end.
export async function selectAllPaged(table, cols, tail) {
  const PAGE = 1000
  const out = []
  for (let offset = 0; ; offset += PAGE) {
    const page = await rest(`${table}?select=${cols}&${tail}&limit=${PAGE}&offset=${offset}`)
...
export const db = {
  ...
  insert: (table, rows) =>
    rest(table, { method: 'POST', body: uniformRows(rows), prefer: 'return=representation' }),
