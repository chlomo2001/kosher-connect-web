...
export async function elidCall(func, params = {}, { hashOrder } = {}) {
  ...
  qs.set('hash', hash)
  const url = `${BASE}/billing/api/${func}?${qs.toString()}`
  const res = await fetch(url, { signal: AbortSignal.timeout(20000) })
  const text = await res.text().catch(() => '')
  // The secret is never in the URL, but strip defensively before returning it.
  return { httpStatus: res.status, ...parseMorXml(text), url: SECRET ? url.split(SECRET).join('') : url }
}
