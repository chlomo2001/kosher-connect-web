...
| Phone inventory (+251px) stacks too, which leaves no list in the app scrolling sideways on a phone. New `.kc-drop-sm`: pool and pool-expiry are a USA-only fact, so on the other rows those two cells leave the card rather than say "N/A" twice | (this commit) | Rentals · phone inventory at ≤560px |
