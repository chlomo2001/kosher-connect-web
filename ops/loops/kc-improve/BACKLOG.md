...
| 08-25 | **One screen, three answers to "how many phones are with customers".** The Rentals tile said **1 currently out** while the banner four lines above it said **3 phones overdue back** — and the tile is a filter button, so pressing it opened a list that did not match the number on it. Three separate counting rules had drifted apart: the tile counted the STORED `status === 'active'` (so every overdue hire — the ones that matter most — was excluded); the list filtered on COMPUTED status (so a rental saved as active but past its end date fell out of the list while still counting on the tile); and `kcNextFacts()` counted every non-returned rental with a passed date as "overdue back", which swept in **booked reservations nobody ever collected** — a phone that never left the shop cannot be overdue back, and it needs a different sentence. Meanwhile `reconcilePhoneStatuses()` had it right all along (`active || overdue`), so the app's own reconciler disagreed with the screen above it. Fixed at all four sites to one rule — a hire counts when a handset is actually with a customer — plus a new `out` filter value so the tile opens exactly what it counted, and the tile relabelled **Phones Out / With customers now** because "Active Rentals" was the word doing the damage. The `!== 'returned'` checks that ask "is this phone COMMITTED" (conflict tests, delete guards, who-holds-this) are deliberately untouched — a reservation does block a phone. **New sweep `ops/harness/counts.mjs`**: renders the tab, reads the number off the tile, presses it, counts the rows, and checks the banner cannot claim more phones overdue than are out at all. Four seconds and green on arrival, so it is wired into `--smoke`. | ✅ gate exit 0 (1115 ×2 TZ + build) + smoke clean incl. the new check + negative control: reintroducing the stored-status count prints `✗ tile says 1, the list it opens holds 2` | owner live-test pending |
| 08-25 | **The same miscount, in a fifth place — and one word for it across the app.** Having fixed the Rentals tile, swept for the pattern: the **Customers tab** carried its own copy, `rentals.filter(r => r.status === 'active')` under the words "Currently out" — the same false sentence on a second screen. Fixed to the same rule. The customer CARD (line 7911) was already correct (`active || overdue`) and was left alone. Then the wording: the app was calling one concept two things after last night's relabel, so **Active Rentals → Phones Out / With customers now** on the Customers tab, the business summary metric and the customer card, matching the Rentals tab. One product, one word for "a handset is with a customer". Verified the fix on the other screen it touches — the dashboard's attention banner and its rentals metric both read 2 against the Rentals tab's 2, where before the banner said 3. | ✅ gate exit 0 (1115 ×2 TZ + build) + smoke clean + rendered dashboard 1280 light and 390 dark + rentals tab at 320px/text-largest (no wrap, no clip, cardfit and overflow both clean) | owner live-test pending |
