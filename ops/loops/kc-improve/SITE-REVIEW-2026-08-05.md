...
CSS/perf/harness layer behind them. 14 review lenses; every "confirmed" item below was
re-read in the code by a second pass before it was written down.
...
#### H · A network blip signs portal customers out and deletes their refresh token
- **Where** `pages/portal.js:229-236` (signOut at `:291-298`)
- **Why** `loadAccount`'s `.catch(() => signOut())` treats a transient fetch failure the same as an
  ...
  filed: `:269` prefers a stale sessionStorage token over the refresh token, so a same-tab reload
  after ~1h expiry deterministically wipes it without ever calling `refreshSession` (`:241-259`).
  Nielsen #9. Costly for this audience — an email round-trip on a filtered phone is expensive.
- **Fix** Sign out only when the server answers `success:false`; on a fetch rejection keep both
  tokens and render a localised "Couldn't load your account — check your connection" card with a
  Try-again button.

#### M · Login form claims "link is on its way" even when the request failed
- **Where** `pages/portal.js:468-481` (success copy `:818-821`, he `:134`)
...
#### M · Expired or already-used sign-in link silently bounces back to the login form
- **Where** `pages/portal.js:261-278`
- **Why** The landing effect reads only `access_token`/`refresh_token` then wipes the hash at `:270`
  ...
  `#error=access_denied&error_code=otp_expired` — never read. Only `pages/auth/google.js:19` (staff)
  handles this. Nielsen #9.
...
#### M · Follow-up enquiries are silently discarded while the sender is told "we've got it"
- **Where** `pages/api/public/message.js:97-117`, `pages/api/public/repair.js:76-96`
- **Why** When an open task with reference `MSG-<sender>` exists the insert is skipped, yet the API
  ...
  moved to Tuesday") never reaches staff. Nielsen #1 — a false success costs the trust the page
  just built.
- **Fix** When an open task matches, append the new submission to that task's `raw_text` via
  `db.update` (timestamped line) instead of skipping the write. Keeps the anti-flood dedup, loses
  nothing. Both endpoints.

#### M · /phone-guide dead-ends on phone calls only, though the shop is open 2:00-6:30pm
- **Where** `pages/phone-guide.js:182` (empty state `:138`, he `:55`)
- **Why** The closing CTA and the empty state are both call-only, and at ≤640px `globals.css:2380`
  hides the back link too — so on a phone the only escape is an undecorated footer wordmark. Fogg
  B=MAT: motivation peaks mid-comparison but the sole trigger fails out of hours, which is most of
  the browsing day.
- **Fix** Add an "or send us a message" link to `/welcome#contact` in `pg-foot` and the empty state
  (en + he). The optional cross-page prefill would need new read-on-mount code in welcome.js — treat
  as separate.

#### M · On phones the call trigger disappears once the hero scrolls away
- **Where** `pages/welcome.js:426-433`; CSS `styles/globals.css:1019-1024`
...
#### M · Phones get no route back to the main site from /phone-guide and /repair
- **Where** `styles/globals.css:2375-2380`; `pages/phone-guide.js:107`, `pages/repair.js:107`
- **Why** The 640px rule was written to hide in-page anchor pills, but on both pages `.w-anchor`
  ...
  (`repair.js:174-176`) is a bare copyright line, and it's the page a customer sits on straight
  after submitting. Nielsen #3/#4 (logo links home).
- **Fix** Wrap the `.w-brand` logo+name in `<a href="/welcome">` on both pages. One JSX wrapper each.

#### M · Visual QA never sees /repair, and only ever sees /phone-guide empty
- **Where** `ops/harness/public.mjs:26` (PAGES); fetch stub `:97-100`
...
#### L · No third-party proof anywhere — the verified Google profile is never linked visibly
- **Where** `pages/welcome.js:123-124` (rendered `:525-530`); cid at `:36`
...
- **Fix** Owner work, not code: ask two or three regulars for one-line named quotes. **Do not** ship a
  ...
  (`welcome.js:596-598` records that decision).
...
#### H · Till and Wallet pick customers from a ~750-option native select with no search
- **Where** `public/main.js:9204-9205, 9234-9237` (POS), `:5599-5601, 5619-5621` (Wallet)
- **Why** Customers are sorted "firstName lastName" (`sortCustomersAZ`, `:262-266`), so native
  type-ahead prefix-matches first names only — surname or phone lookup means scrolling ~751 options.
  Particularly weak for this audience, where repeated first names are the norm and the surname is
  the discriminator. Hick's Law; Baymard (>10-15 options need search). The Wallet half has partial
  escapes (Ctrl-K, the customer card's own Record button, inline arrears rows); **the POS half has
  none, and a mis-pick charges the wrong customer's wallet** (`:9881`).
- **Fix** Reuse the rental type-ahead picker (`:717-748` — name/phone/email/digit search, capped at
  10) for `#posCustomer` and `#wtCustomer`; parameterise its element IDs so all three share one
  component. Keep a hidden input with `id="posCustomer"` and all five existing `.value` reads
  (`:9285, 9535, 9578, 9846, 9881`) keep working.

#### M · Bank reconciliation blanks to a skeleton and loses your place after every action
- **Where** `public/main.js:5909, 5950, 5959, 5977` → `renderBankRecon()` `:5739-5740`
...
- **Fix** Patch the row state and counts inside `bankData`, call `bankPaint()`, refetch quietly in
  the background (still needed for the `proposeCapped` proposal window).

#### M · New Rental dead-ends when the renter isn't a customer yet
- **Where** `public/main.js:737-739` (only zero-match branch); picker markup `:1866-1876`
...
#### M · Customer-card tool strip is ten unlabelled emoji buttons, including a charge-card action
- **Where** `public/main.js:4012-4028`; `ops/harness/modal_customer-card_light_390.png`
- **Why** Meanings live only in hover titles (no touch tooltip), and several emoji degrade to
  ...
  usability; Nielsen #6. *`💳` is already gated behind a `prompt()` and an idempotency ref, so an
  accidental tap charges nothing — the cost falls on an occasional new helper, not the daily
  operator.*
...
#### M · Palette match is exact-substring, so natural phrasings find nothing
- **Where** `public/main.js:10781-10797`
- **Why** `label.toLowerCase().includes(needle)` means "cash up", "cashup" and "z report" return
  ...
  / Recent / numbered quick actions) are hidden the moment you type (`:11048`). Nielsen #6; Baymard
  search findings.
- **Fix** Strip non-alphanumerics from both needle and label before matching, and add an optional
  `keys:[]` alias array per command (pos, till, eod, z report, end of day). The same function
  already normalises phone digits, so this matches the file's own pattern.

#### M · Kol Torah is invisible to the command palette
- **Where** `public/main.js:10768` (hand-written 12-id array) vs `TAB_META` `:599-613`
- **Why** Every tab id except `koltorah` is listed, so "kol" offers neither navigation nor "+ New
  Job" (`ktFocusNewJob`, `:10138`) — while the comment directly above at `:10766-10767` claims these
  entries read the same label map. Nielsen #4.
- **Fix** Derive the navigate entries from `Object.keys(TAB_META)` so the next new tab can't be
  forgotten, and add a create command reading koltorah's existing primary. `visibleCommands()`
  already filters by `c.tab` against `allowedTabs`, so helper permissions are preserved.

#### M · Assistant's "today's takings" silently dead-ends on a non-existent tab
- **Where** `public/main.js:10597` → `goToTab()` `:11751-11756`
- **Why** It prints "Opening the cash-up screen…" then clicks `.nav-item[data-tab="cashup"]`, which
  doesn't exist — optional chaining swallows the miss, the modal closes, nothing opens. Verified
  against the rendered DOM in `ops/harness/app.html`. Fires every time. Nielsen #1.
- **Fix** One line: `openCashupModal()` (`:5650`), already called the same way from `:5624` and
  `:10740`.

#### M · Cash-up can only count today — a missed close-out becomes unrecordable
- **Where** `public/main.js:5650-5652, 5665, 5695`; both call sites (`:5624`, `:10740`) are zero-arg
- **Why** `localISO()` is hardcoded for fetch, title and save, and there's no date control
  (`modal_cashup_light_390.png` shows the date as static text). The API already accepts any date
  (`pages/api/cashup.js:67, 89`), so the restriction is purely client-side. Nielsen #5 — leave a
  recovery path for a predictable human miss. *Correction: no variance rolls forward
  arithmetically; the harm is that the missed day gets no `till_counts` row and the next physical
  count reads unexplainably over.*
- **Fix** Give `openCashupModal` a date parameter plus a "‹ previous day" stepper in the title row,
  passed through to GET and POST. While there, bind Enter in `#cuCounted` to `saveCashup`.

#### M · Charging a sale has no keyboard path — the till's most-repeated action needs the mouse
- **Where** `public/main.js:9216-9218, 9399-9402` (scan field), `:9245` (Charge button)
- **Why** Every POS action deliberately refocuses the scan field (`:9254, 9268, 9340`), Enter on an
  ...
  button, so Tab-then-Enter reaches it; what's missing is a direct shortcut. The till is also
  deliberately touch-first, so a pointer is often already in play.*
- **Fix** Map Ctrl+Enter (or F9) in the scan field's keydown to `saveSale()` — a modifier stops a
  scanner's trailing Enter firing the confirm-free charge — and show the shortcut on the button.

#### L · The global-search button is labelled by its keyboard chord, "Ctrl K"
- **Where** `public/main.js:6039-6047`; `ops/harness/shot_dashboard_light_390.png`
- **Why** The word "Search" exists only in the hover title, and at 390px the topbar reads
  "🤖 Ask / 🔍 Ctrl K / 🌙" on a device with no Ctrl key. The palette is the only find-anything entry
  on 12 of 13 tabs (`search:true` occurs once, `:601`). Nielsen #2.
- **Fix** Label it "Search" and move the `Ctrl K` hint into a span hidden below 768px.

#### L · Empty `helper_tabs` signs a helper into a blank app with only a transient toast
- **Where** `lib/auth.js:198-211` → `public/main.js:591, 361-363, 629-631`
- **Why** Fails closed to `[]`, which hides every nav item, makes `startTab` undefined and bails with
  a toast — blank app, no persistent explanation. Nielsen #9. *Unreachable through the product:
  migration `20260712121900_helper_tabs.sql` seeds 12 tabs, `saveHelperTabs()` (`:13452`) and
  `pages/api/settings.js:174` both refuse an empty selection. Only a bad restore gets you there.*
...
#### M · Customer card opens as a modal with no dialog role, focus move-in, Tab trap or focus return
- **Where** `public/main.js:4159`; `kcTopModalOverlay` `:11336`; `dismissCustomerCard` `:4900`
- **Why** A bare `<div class="modal">` with no role/aria-modal/name; `customerCard` is absent from
  ...
  `role=dialog` — this one contradicts it. WCAG 4.1.2, 2.4.3; ARIA APG. Escape does work (`:11145`).
- **Fix** Mirror `kcConfirm` (`:3251`): `role="dialog" aria-modal="true"` plus an aria-label with the
  customer's name; append `customerCard` to `kcTopModalOverlay`; save/restore focus.
  **Caveat:** `renderDetailPanel` is also the repaint path (every save handler re-enters it — see the
  `wasOpen` guard at `:4161`), so the focus save and move-in must be gated on `!wasOpen`, or a
  mid-edit save will clobber the return-focus record and yank focus back to the card.

#### M · Opening hours stay in English on the Hebrew welcome page
- **Where** `pages/welcome.js:284, 312, 611`; `pages/api/public/info.js:8, 15`
- **Why** The label is Hebrew ("שעות פתיחה") but the value prints raw inside the `dir="rtl"` wrap,
  ...
  (`:605, 615, 619`), which do get `dir="ltr"`. There is no `opening_hours_he` key anywhere, so the
  owner can't fix it from Settings without breaking the EN page. Nielsen #2.
- **Fix** Add an `opening_hours_he` settings key edited beside `opening_hours` (clone the bespoke
  branch at `pages/api/settings.js:213-224`); render it when `lang==='he'`, falling back to the EN
  string wrapped in `dir="ltr"`. Leave the JSON-LD copy (`:272`) English.

#### L · All 49 dynamic action modals announce as nameless dialogs
- **Where** `public/main.js:3176`
- **Why** `role="dialog" aria-modal="true"` with no `aria-labelledby`, though every payload opens
  ...
  (`:3183`) moves focus into the first input, so the visible title is never announced at all.
  `kcConfirm` (`:3251`) and `kcShortcuts` (`:11097`) do it right. WCAG 4.1.2.
- **Fix** After setting innerHTML, find the first `.modal-title`, give it an id, and set
  `aria-labelledby` on the `.modal` div. One function fixes all 49. (The `role="heading"` half of the
  original suggestion is unnecessary.)

#### L · Staff shell has zero landmarks, no h1, and the active tab isn't exposed
- **Where** `components/AppShell.js:78, 88, 128, 129, 133, 148`; `syncNavActive` `public/main.js:477`
- **Why** All plain divs — no nav/main/header/h1 anywhere in the shell, one h2 and two h3 in 13,694
  lines of main.js, no `aria-current`, no skip link. WCAG 1.3.1, 2.4.1. Internal staff surface with
  no known AT user, so this is insurance rather than live friction.
- **Fix** Attribute-only, zero CSS risk: `role="navigation" aria-label="Main"` on `.sidebar-nav`,
  ...
  `#pageTitle`, and `aria-current="page"` alongside `.active`. Note the nav items are
  `div.nav-item` (`AppShell.js:93`), so they aren't focusable at all — a larger gap this doesn't fix.

#### L · Persistent error toasts can only be dismissed with a mouse
- **Where** `public/main.js:7035-7053`
- **Why** Error toasts deliberately never auto-clear, but dismissal is an `addEventListener` click on
  ...
  `[onclick]` attributes. Escape doesn't cover toasts either (`:11138-11146`). WCAG 2.1.1. Two
  mouse-using staff, and the toast sits in a fixed corner without blocking the form, so live impact
  is near zero.
- **Fix** Append a real `<button aria-label="Dismiss">` inside the toast. One function serves all 186
  error call sites.

#### L · Type-ahead controls have no combobox semantics; the palette drops focus on close
- **Where** `public/main.js:11023-11061, 10875-10886, 11064-11066, 11335-11341`; picker `:717-748`
- **Why** Arrowing moves a CSS class over onclick divs with no `role=listbox/option`,
  no `aria-activedescendant`, no result-count live region — a screen reader hears nothing.
...
- **Fix** The one part that bites a sighted keyboard user daily is the missing focus restore on Esc —
  wrap open/close in the existing `kcSaveReturnFocus`/`kcRestoreReturnFocus` helpers. The full APG
  combobox rework (roles, ids, activedescendant, "N results" live region) is a quiet-day job.
...
#### L · Modal Save/Cancel row grazes the iOS Safari toolbar
- **Where** `styles/globals.css:818-825` (`.modal`), `:795-800` (`.modal-overlay`)
- **Why** `vh` resolves to the large viewport, so with toolbars shown a maxed-out modal is taller
  ...
  On an iPhone 14 that's ~30px at the bottom — almost entirely inside the modal's own 28px padding.
  *The original framing is refuted: `.modal` carries `overflow-y: auto` on the very next line, so it
  IS the scroll container and Save/Cancel is always reachable by scrolling. The login-card fix at
  `:1533-1535` is a different defect (adding scrolling, not dvh).*
- **Fix** Cheap and harmless: `max-height: 90vh; max-height: 90dvh;` progressive pair, plus
  `align-items: safe center` on the overlay.
...
#### H · Till goes silent for the whole card-machine charge
- **Where** `public/main.js:9245` (button), `:9825` (single 3s toast), `:9717`/`:9696-9707` (waits),
  `:26` (`kcBeginWrite` returns false silently)
- **Why** One info toast that auto-clears in 3s, then a poll of up to 180s with no busy indicator;
  ...
  tap/PIN/issuer round trip is ~10-40s — well past the ~10s persistent-indicator threshold. The
  operator cannot tell "waiting on the machine" from "my tap didn't register". ~12 other places in
  the file already do `btn.disabled = true; btn.textContent = 'Creating…'`, so this button is the
  outlier. Nielsen #1; Doherty. *No money risk: `kcBeginWrite` + the `posSaleRef` idempotency token
  (`:9815`) + the amount-matched `kcTillApproved` cache (`:9689-9693`) prevent a double charge.*
- **Fix** In `saveSale`, disable `.pos-charge` and relabel it "Waiting for the card machine…" while
  awaiting `kcTillCharge` (restore in `finally`), and make `kcBeginWrite` toast "Still working on the
  last one…" when it blocks a tap.

#### M · Failed customer save closes the modal silently — typed data lost, staff believe it saved
- **Where** `public/main.js:6301-6309` (`closeModal()` at `:6309` runs unconditionally); `clearModal()`
  `:6143`
- **Why** Neither the add nor the update branch has a failure path. `pages/api/customers.js` returns
  ...
  leaves an unhandled rejection through `guardReentry` (`:13634`). Every sibling path handles it:
  `deleteCustomer` (`:6329-6333`), `saveCashup` (`:5719`), `reportSave` (`:1201`). The repo's own
  task #24 set this convention; this form was missed. Nielsen #1/#5; Baymard (preserve input on
  error).
...
#### L · Staff sign-in defeats password managers and code autofill
- **Where** `pages/login.js:111-120` (email/password), `:95-99` (2FA code)
- **Why** No `name`, no `autoComplete`, no labels beyond placeholders; the 2FA input has
  ...
  (`portal.js:827`, `welcome.js:538`, `repair.js:145`) — login.js is the one holdout. WCAG 1.3.5.
  *Fill isn't actually broken — browsers detect `type="password"` heuristically; the attributes make
  it deterministic. `one-time-code` buys little for an emailed code on a desktop.*
...
#### L · /repair's "Phone or email" field opens an email keyboard
- **Where** `pages/repair.js:149`
- **Why** `autoComplete="tel"` paired with `inputMode="email"` — autofill offers a phone number while
  ...
  *Half the original finding is refuted: `pages/welcome.js:540`'s bare default keyboard is exactly
  the correct end state, not a second defect, since a field accepting either has no right
  `inputMode`.*
- **Fix** Drop `inputMode="email"` from `repair.js:149`, keep `autoComplete="tel"`. Add `name`
  attributes to the four repair fields while you're there (autofill heuristics only; nothing
  functional breaks without them).
...
#### M · main.js ships 764KB unminified with no long-cache
- **Where** `next.config.js:32-36`; loaded at `components/AppShell.js:265`
- **Why** 779,773 bytes raw / 201KB gzipped on a stable path at Next's default `max-age=0`; the
  config's own NOTE flags this as a deferred owner-verify item. Repeat boots pay a conditional
  request, and every deploy re-downloads the lot on old till hardware. *Two corrections: terser 5
  gives 31% off the wire (201KB → 139KB), not "roughly half"; and `max-age=0` means a 304 with no
  body, on an `afterInteractive` tag that is not render-blocking — so "every boot pays a blocking
  RTT" overstates it.*
- **Fix** Do the caching half now: reference `/main.js?v=<BUILD_ID>` from `AppShell.js:265` plus one
  immutable `headers()` rule. One line each, no new toolchain, and `ops/harness/render.mjs:63` (which
  reads `public/main.js` by literal path) keeps working. Leave the terser step for the owner-present
  deploy check the config already asks for — main.js is hand-edited near-daily and would need a
  source-map story.

#### M · Boot splash blocks first paint on the whole API load
- **Where** `public/main.js:314-326` (11-way `Promise.all`), `hideBootLoader()` `:376`
- **Why** Staff watch "Loading your business…" until the slowest serverless call returns, including
  ...
  (`:6962`) and the dashboard's paint-from-cache pattern (`:11779-11800`) already exist. A cold
  Vercel start makes it long enough that `AppShell.js:269` carries a 12-second escape hatch.
  *Corrections: the fetches are parallel, so it's one round trip, not eleven; and the payoff is
  purely perceptual — every tab reads the arrays this fills, so staff still can't look anything up
  any sooner.*
- **Fix** Paint the chrome plus the start tab's skeleton and call `hideBootLoader()` immediately;
  render the real tab when the `Promise.all` resolves. **Budget for one edge case the original
...
#### M · /repair sets no expectations: no price anchors, no firm turnaround
- **Where** `pages/repair.js:25` (EN), `:45` (HE)
- **Why** The only expectation-setting is "we'll come back with an honest price… Most jobs are done
  ...
  both languages; the success card already gives drop-off address and bell-5 wayfinding; and the
  Baymard abandonment research doesn't apply — /repair is a free enquiry that queues a staff task,
  not a checkout, so the failure mode is hesitation, not lost payment.*
...
#### M · Repair confirmation says "bring it in whenever suits you"; the shop opens 2:00-6:30pm
- **Where** `pages/repair.js:31` (EN), `:51` (HE); hours at `pages/api/public/info.js:8`
- **Why** Hours appear on /welcome alone (`welcome.js:310-313, 611`) — nowhere on /repair or
  /phone-guide. The real problem isn't the Friday/Shabbos closure, which this audience assumes; it's
  the afternoon-only window, which makes "whenever suits you" invite a wasted morning trip.
  Nielsen #2.
- **Fix** Fetch `/api/public/info` exactly as `welcome.js:310-313` does and replace "whenever suits
  you" with the hours line, so the owner's Settings value stays the single source of truth. The
  /repair half is load-bearing; the /phone-guide footer addition is optional.
...
| Verified in code | 35 confirmed (3 high, 21 medium, 11 low after verifier re-ranking) |
