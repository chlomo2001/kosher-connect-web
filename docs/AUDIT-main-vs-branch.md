...
### On the branch (.gs)
1. ~~[CRITICAL] `Ledger.gs` and `CustomerOnboarding.gs` missing~~ **RESOLVED 2026-07-12** — both added to the repo from the live project. Note the live project still has files not in the repo: `WebConsole.gs` (`addCustomerFromConsole` — the live customer-creation path), `TodoSMS.gs` (the single `doPost` web-app router: Twilio SMS + Stripe webhook), `PoolOptimiser.gs`.
2. ~~[CRITICAL] Charge-sign contradiction~~ **RESOLVED — benign.** `appendLedgerEntry` takes `Math.abs(amount)` and derives the sign from `typeSign_(type)`, so Bookings' negative and Repairs' positive both posted correctly. Bookings.gs cleaned to pass the magnitude. Two real collisions found instead and fixed: `ledgerHasReference_` and `nextSequentialId_` were each defined in both Bookings.gs and the pasted files (Apps Script global namespace — last-loaded silently wins); Bookings.gs's copies removed, canonical versions kept (Ledger.gs / CustomerOnboarding.gs).
2b. **[HIGH → FIXED]** `autoDeduct` has no idempotency guard and SIMWatcher passed no reference — an error after the deduction left the mail unread, and the retry deducted the fee again. Now deduped on `SIMRENEW-<gmail message id>` via `ledgerHasReference_(ref, 'Auto-deduct')`, the convention Ledger.gs documents.
2c. **[FIXED]** Code.gs Tasks SCHEMA lacked the `Reference` column that `runCollectionsReminders`/`upsertTaskByReference_` hard-require as the `BALANCE-<id>` dedup key — appended last so positional 8-col writers stay compatible.
2d. **[NOTE]** `getWalletBalance` matches CustomerID with strict `===` — a string-typed id cell (AppSheet writes) silently sums to £0. Auto-ID logic now exists three times per edit (Code.gs simple `onEdit` + its installable twin + CustomerOnboarding's `onEditTrigger`) — idempotent but wasteful; consolidate.
