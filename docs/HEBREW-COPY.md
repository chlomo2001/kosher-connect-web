# The Hebrew, next to the English it stands for

**Generated** by `scripts/build-hebrew-review.mjs` — do not edit by hand.
Re-run it after any copy change and the file catches up.

Every line of Hebrew the shop puts in front of a customer is below, beside
the English it translates and the screen it appears on. Anything wrong can be
answered with the key in the first column — that is the exact place in the
source, so a correction can be made without hunting for it.

Yiddish and Hebrew are not distinguished here: the pages offer one non-English
language and this is all of it.
...
| `bands.1.cta` | Plan your trip | דברו איתנו לפני הטיסה |
| `bands.1.prefill` | I’m planning a trip and would like a travel phone. Where I’m going and when: | אני מתכנן/ת נסיעה ואשמח לשכור טלפון לחו״ל. לאן ומתי: |
| `bands.2.eyebrow` | International numbers | מספרים בינלאומיים |
| `bands.2.title` | An Israeli or USA number that rings on your UK phone | מספר ישראלי או אמריקאי — מצלצל אצלכם בנייד |
| `bands.2.accent` | Same day | באותו היום |
| `bands.2.sub` | an Israeli or USA number, ringing on your own phone | המספר מקומי, הטלפון — שלכם |
| `bands.2.body` | Family in Israel? Business in the States? Get a local number that rings straight through to the phone already in your pocket — no second handset, and no roaming charges for the people calling you. | המשפחה בארץ? עסקים באמריקה? מקבלים מספר מקומי שמצלצל ישר לנייד שכבר יש לכם — בלי מכשיר נוסף, ומי שמתקשר אליכם משלם שיחה מקומית רגילה, בלי רומינג. |
| `bands.2.chips.0` | Rings on your own phone | מצלצל בנייד שלכם |
| `bands.2.chips.1` | Israeli or USA number | מספר ישראלי או אמריקאי |
| `bands.2.chips.2` | No roaming for callers | שיחה מקומית למתקשרים |
| `bands.2.price` | from £10 a month | החל מ־‎£10‎ לחודש |
| `bands.2.cta` | Get your number | להזמנת מספר |
| `bands.2.prefill` | I’d like an Israeli or USA number that rings on my own phone. | אשמח למספר ישראלי או אמריקאי שמצלצל אצלי בנייד. |
...
| `moreGroups.0.items.1.prefill` | I’m after an accessory — what I need: | אני מחפש/ת אביזר — מה שאני צריך/ה: |
| `moreGroups.0.items.2.title` | Kosher phones | טלפונים כשרים |
| `moreGroups.0.items.2.body` | The right kosher handset for you — we stock a range and talk you through what suits. | מגוון מכשירים כשרים במלאי — ונעזור לכם לבחור את מה שבאמת מתאים. |
| `moreGroups.0.items.2.linkLabel` | See the phone guide → | למדריך הטלפונים ← |
| `moreGroups.1.label` | Everyday help | עזרה יומיומית |
| `moreGroups.1.items.0.title` | Kol Torah audio | קול תורה |
| `moreGroups.1.items.0.body` | Shiurim, music and children’s stories — on CD or loaded onto your phone or player while you visit. | שיעורים, ניגונים וסיפורים לילדים — על דיסק, או שנעביר לכם ישר לטלפון או לנגן בזמן שאתם כאן. |
| `moreGroups.1.items.0.linkLabel` | Ask about Kol Torah → | לשאול על קול תורה ← |
| `moreGroups.1.items.0.prefill` | I’d like to ask about Kol Torah audio — shiurim, music or children’s stories. | אשמח לשמוע על קול תורה — שיעורים, ניגונים או סיפורים לילדים. |
| `moreGroups.1.items.1.title` | Flights & tickets | טיסות וכרטיסים |
| `moreGroups.1.items.1.body` | We book the trip and keep an eye on times and changes, so it’s sorted before you pack. | מזמינים לכם את הטיסה ועוקבים אחרי הזמנים והשינויים — הכול סגור עוד לפני שאתם אורזים. |
| `moreGroups.1.items.1.linkLabel` | Start a booking → | להתחיל הזמנה ← |
| `moreGroups.1.items.1.prefill` | I’d like help booking a flight. Where I’m going and when: | אשמח לעזרה בהזמנת טיסה. לאן ומתי: |
| `moreGroups.1.items.2.title` | Online help | עזרה אונליין |
| `moreGroups.1.items.2.body` | Forms, printing and the little online jobs that are easier done properly than fought with. | טפסים, הדפסות וכל הסידורים הקטנים באינטרנט — אנחנו נעשה את זה בשבילכם, בלי כאב ראש. |
| `moreGroups.1.items.2.linkLabel` | Tell us what you need → | ספרו לנו מה צריך ← |
| `moreGroups.1.items.2.prefill` | I could use a hand with an online job — here’s what: | אשמח לעזרה בסידור באינטרנט — במה מדובר: |
| `phoneGuideCta` | Choosing a handset? See the phone guide → | מתלבטים איזה מכשיר? למדריך הטלפונים ← |
| `repairCta` | Something broken? Book a repair → | משהו התקלקל? לקבוע תיקון ← |
| `friendsEyebrow` | Why people send their friends | למה ממליצים עלינו |
| `friendsText` | No contracts, no middleman, no jargon — just the deal that actually fits you. And if you’re already on a good one, we’ll tell you. | בלי חוזים, בלי מתווכים, בלי אותיות קטנות — פשוט מה שמתאים לכם באמת. וכשאתם כבר על חבילה טובה? נגיד לכם לא לגעת. |
...
| `fReach` | Best way to reach you (optional) | איך הכי נוח לחזור אליכם? (לא חובה) |
| `fReachOpts.call` | Call | שיחת טלפון |
| `fReachOpts.text` | Text message | הודעת טקסט |
| `fReachOpts.whatsapp` | WhatsApp | וואטסאפ |
| `fReachOpts.email` | Email | אימייל |
| `fOk` | Thanks — we’ve got it. We reply during opening hours, usually the same day. | תודה! ההודעה אצלנו — נחזור אליכם בשעות הפתיחה, לרוב עוד באותו יום. |
| `fErrLead` | Couldn’t send — please call us on | ההודעה לא נשלחה — התקשרו אלינו: |
| `fErrCodes.rate` | That’s a few messages in a row — give it a minute, or call us on | נשלחו כמה הודעות ברצף — נסו שוב בעוד דקה, או התקשרו אלינו: |
| `fErrCodes.name` | Please enter your name, then try again — or call us on | נא למלא את השם ולנסות שוב — או להתקשר אלינו: |
| `fErrCodes.contact` | Please check the phone number or email, then try again — or call us on | נא לבדוק את מספר הטלפון או המייל ולנסות שוב — או להתקשר אלינו: |
| `fErrCodes.offline` | We can’t take messages this minute — please call us on | לא נוכל לקבל הודעות ברגע זה — נא להתקשר אלינו: |
| `fErrCodes.server` | Couldn’t send — please call us on | ההודעה לא נשלחה — התקשרו אלינו: |
...
| `faq.1.a` | Yes, in almost every case. You ask your current provider for a PAC code — it is free, and they must give it to you — and we do the rest. Your number stays yours, and we will tell you before anything switches over so you are never without a phone unexpectedly. | כן, כמעט תמיד. מבקשים מהחברה הנוכחית קוד PAC — זה בחינם והם חייבים לתת לכם אותו — ואת השאר אנחנו עושים. המספר נשאר שלכם, ונעדכן אתכם לפני שמשהו עובר, כך שלא תישארו בלי טלפון בהפתעה. |
| `faq.2.q` | Can I have an American or Israeli number that rings here in England? | אפשר מספר אמריקאי או ישראלי שמצלצל כאן באנגליה? |
| `faq.2.a` | Yes — that is what our international numbers do. Family or business abroad dial a local number in their country, and it reaches you here. Ring us and we will explain which countries we cover and what suits how you will use it. | כן — בדיוק בשביל זה המספרים הבינלאומיים. המשפחה או העסק בחו״ל מחייגים מספר מקומי אצלם, וזה מגיע אליכם לכאן. התקשרו ונסביר אילו מדינות יש לנו ומה מתאים לאופן שבו תשתמשו בזה. |
| `faq.3.q` | I am travelling — what should I sort out before I go? | אני נוסע — מה כדאי לסדר לפני? |
| `faq.3.a` | Come in before you fly and we will set a rental phone up with you in the shop, so there is nothing to arrange at a foreign airport. Shabbos and Yom Tov are never charged. If you are flying with us as well, we can book the flight and the phone in one visit. | בואו לפני הטיסה ונכין איתכם את הטלפון בחנות, כך שלא צריך לארגן כלום בשדה תעופה זר. שבת ויום טוב אף פעם לא מחויבים. אם גם את הטיסה מזמינים דרכנו, אפשר לסגור את שניהם בביקור אחד. |
| `faq.4.q` | My new SIM is not working — is it broken? | הסים החדש לא עובד — הוא תקול? |
| `faq.4.a` | Nearly always not. It is usually the phone needing a restart after the SIM goes in, or the plan not being switched on yet. Give us a ring and we will check it from our side before you make a journey. | כמעט תמיד לא. בדרך כלל צריך לכבות ולהדליק את הטלפון אחרי הכנסת הסים, או שהחבילה עוד לא הופעלה. התקשרו ונבדוק מהצד שלנו לפני שתגיעו במיוחד. |
| `faq.5.q` | My phone stopped working while I was abroad — what do I do? | הטלפון הפסיק לעבוד בחו״ל — מה עושים? |
| `faq.5.a` | Ring us on 0161 531 1386. If it is a rental we can usually sort it over the phone, and if it cannot be fixed remotely we will arrange a replacement. Do not buy a local SIM before speaking to us — it often makes the number harder to recover. | התקשרו אלינו: 0161 531 1386. אם זו השכרה, לרוב נוכל לפתור את זה בטלפון, ואם לא — נדאג למכשיר חלופי. אל תקנו סים מקומי לפני שדיברתם איתנו; זה בדרך כלל מקשה על שחזור המספר. |
| `faq.6.q` | How do I pay, and what happens if I forget? | איך משלמים, ומה קורה אם שוכחים? |
...
| `waLabel` | WhatsApp us | כתבו לנו בוואטסאפ |
| `waText` | Hello Kosher Connect — I’d like to ask about… | שלום כשר קונקט — רציתי לשאול לגבי… |
| `visitTitle` | Come and see us | מחכים לכם בחנות |
| `openMaps` | Open in Google Maps | לניווט בגוגל מפות |
| `addressLabel` | Address | כתובת |
| `findUs` | The door left of Toy Zone (MMR Group sign) — ring bell 5, we’re on level 2. | הדלת משמאל ל־Toy Zone (שלט MMR Group) — מצלצלים בפעמון 5, קומה 2. |
| `phoneLabel` | Phone | טלפון |
| `emailLabel` | Email | מייל |
| `footServices` | Services | שירותים |
| `footAccount` | Your account | האזור האישי |
| `footLegal` | Information | מידע |
...
| `paidTitle` | Payment received — thank you! | התשלום התקבל — תודה רבה! |
| `paidBody` | Your payment went through and will show on your account shortly. | התשלום עבר בהצלחה ויופיע בחשבון שלכם בקרוב. |
| `paidClose` | Dismiss | סגירה |
| `backTop` | Back to top | חזרה למעלה |
...
| `processing` | Processing… | התשלום מתבצע… |
| `cancel` | Cancel | ביטול |
...
| `noMatchBody` | You’re signed in, but this email address isn’t linked to a Kosher Connect account yet. If you’re a customer, we may have a different email (or none) on file — call us and we’ll link it up in a minute. | נכנסתם בהצלחה, אבל כתובת המייל הזו עדיין לא מקושרת לחשבון בכשר קונקט. ייתכן שרשומה אצלנו כתובת אחרת (או שאין בכלל) — התקשרו אלינו ונקשר את החשבון תוך דקה. |
| `tryAnother` | Try a different email | לנסות כתובת מייל אחרת |
| `statement` | Recent activity | תנועות אחרונות |
| `noStatement` | No activity yet. | עוד אין תנועות להצגה. |
| `docs` | Documents | מסמכים |
| `noDocs` | Nothing shared with you yet. | עוד לא שותפו איתכם מסמכים. |
| `download` | Download | הורדה |
| `upload` | Send us a document | שליחת מסמך אלינו |
| `uploading` | Uploading… | המסמך בדרך… |
| `docSend` | Send this file | שליחת הקובץ |
| `upSent` | Sent — we’ll review it shortly. | הגיע אלינו — נעבור עליו בקרוב. |
| `upFailed` | Upload failed. | ההעלאה לא הצליחה. נסו שוב. |
| `pendingReview` | Awaiting review | ממתין לבדיקה |
...
| `statuses.Ticketed` | Ticket issued | כורטסה |
| `statuses.Confirmed` | Confirmed | מאושרת |
| `statuses.Cancelled` | Cancelled | בוטלה |
| `statuses.Completed` | Completed | הושלמה |
| `daysLeft()` | … days left | נותרו … ימים |
| `subSignedOut()` | …! See your rentals, bookings and balance | …! כל ההשכרות, ההזמנות והיתרה שלכם — במקום אחד |
| `yourEmail` | Your email | כתובת המייל שלכם |
| `emailLink` | Email me a sign-in link | שלחו לי קישור כניסה |
...
| `entryTypes.manual_adjustment` | Adjustment | התאמה |
| `entryTypes.topup` | Top-up | טעינה |
| `entryTypes.rental` | Rental | השכרה |
| `entryTypes.sim_charge` | SIM plan | חבילת SIM |
| `entryTypes.booking` | Flight booking | הזמנת טיסה |
| `entryTypes.repair` | Repair | תיקון |
| `entryTypes.service` | Service | שירות |
| `balAfterOwed()` | owed … after this | נותרה יתרת חוב … אחרי הפעולה |
| `balAfterCredit()` | … in credit after this | נותרה יתרת זכות … אחרי הפעולה |
| `sent` | 📬 If that email belongs to a Kosher Connect customer, a sign-in link is on its way. You can close this page. | 📬 אם הכתובת שייכת ללקוח של כשר קונקט — קישור הכניסה כבר בדרך אליכם. אפשר לסגור את העמוד. |
| `linkExpired` | That sign-in link has expired — enter your email and we’ll send a fresh one. | תוקף קישור הכניסה פג — הזינו את כתובת המייל ונשלח לכם קישור חדש. |
| `sendFailed` | That didn’t send — try again in a minute, or call us on | השליחה לא הצליחה — נסו שוב בעוד דקה, או התקשרו אלינו: |
| `offline` | We couldn’t reach us just now — check your connection and try again, or call us on | לא הצלחנו להתחבר כרגע — בדקו את החיבור ונסו שוב, או התקשרו אלינו: |
| `netErrTitle` | We couldn’t load your account | לא הצלחנו לטעון את החשבון שלכם |
| `netErrBody` | That looks like a connection problem, not a sign-in problem — you’re still signed in. | נראה שזו בעיית חיבור ולא בעיית התחברות — אתם עדיין מחוברים. |
| `tryAgain` | Try again | לנסות שוב |
| `or` | or | או |
| `google` | Continue with Google | כניסה עם Google |
| `greeting()` | Good evening | ערב טוב |
| `greeting()` | Good morning | בוקר טוב |
| `greeting()` | Good afternoon | צהריים טובים |
| `title` | My Kosher Connect | כשר קונקט שלי |
| `phoneFallback` | Phone | טלפון |
| `flightFallback` | Flight | טיסה |
| `backToSite` | Back to kosher-connect.com | חזרה לאתר כשר קונקט |
...
Hebrew outside the four bilingual pages, with no English twin: the company name,
the Hebrew month names, and the few places the STAFF app shows Hebrew. Lower
priority than the tables above — no customer reads most of it — but the month
names in particular are worth a glance, since every Hebrew date in the app is
built from them.

| File | Line | Hebrew |
| --- | --- | --- |
| `lib/hebrewDate.mjs` | 100 | const HE_ONES = ['', 'א', 'ב', 'ג', 'ד', 'ה', 'ו', 'ז', 'ח', 'ט'] |
| `lib/hebrewDate.mjs` | 101 | const HE_TENS = ['', 'י', 'כ', 'ל', 'מ', 'נ', 'ס', 'ע', 'פ', 'צ'] |
| `lib/hebrewDate.mjs` | 102 | const HE_HUNDREDS = ['', 'ק', 'ר', 'ש', 'ת'] |
| `lib/hebrewDate.mjs` | 104 | /** 1..999 as a Hebrew numeral with geresh/gershayim; 15/16 spelt ט״ו/ט״ז. */ |
| `lib/hebrewDate.mjs` | 109 | while (v >= 400) { s += 'ת'; v -= 400 } |
| `lib/hebrewDate.mjs` | 111 | if (v === 15) { s += 'טו'; v = 0 }        // avoid spelling the Name |
| `lib/hebrewDate.mjs` | 112 | else if (v === 16) { s += 'טז'; v = 0 } |
| `lib/hebrewDate.mjs` | 115 | return s.length > 1 ? s.slice(0, -1) + '״' + s.slice(-1) : s + '׳' |
| `lib/hebrewDate.mjs` | 118 | const MONTHS_HE = ['', 'ניסן', 'אייר', 'סיון', 'תמוז', 'אב', 'אלול', |
| `lib/hebrewDate.mjs` | 119 | 'תשרי', 'חשון', 'כסלו', 'טבת', 'שבט', 'אדר', 'אדר ב׳'] |
| `lib/hebrewDate.mjs` | 124 | if (hm === 12 && isHebrewLeapYear(hy)) return lang === 'he' ? 'אדר א׳' : 'Adar I' |
| `lib/hebrewDate.mjs` | 129 | * 'ה׳ אלול תשפ״ו' (or '5 Elul 5786') for a civil date. Accepts (gy,gm,gd) |
| `lib/hebrewDate.mjs` | 148 | /** Hebrew weekday letter for a civil date — 'ג׳' for a Tuesday; Shabbos 'שבת'. */ |
